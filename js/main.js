...
function getHeroImage() {
  ...
  const named = list.find((img) => img.isHero);
  return named || list[0];
}
...
function initHeroBg() {
  ...
  if (!imgData) {
    ...
    const old = visual.querySelector("img.hero-photo");
    if (old) old.remove();
    return;
  }
  ...
  if (!photo) {
    ...
    photo.addEventListener("error", () => {
      // битый файл — возвращаемся к тёмному fallback
      photo.remove();
      hero.classList.remove("has-photo");
    });
    visual.insertBefore(photo, visual.firstChild);
  }
  photo.src = imgData.src;
  photo.alt = imgData.alt || "ZAGA GAME";
}
